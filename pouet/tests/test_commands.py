...
ASTRAL_BLUR = {
    ...
    "addeduser": {
        ...
        "avatar": "bobble.gif",
        "glops": "850",
        "registerDate": "2000-06-25 17:59:58",
    },
    "sceneorg": "0",
    "demozoo": "11",
    "csdb": "0",
    ...
    "rank": "665",
    "cdc": 0,
    "downloadLinks": [{"type": "youtube", "link": "https://www.youtube.com/watch?v=eZyLSHyUGBY"}],
    "credits": [
        {
            "user": {
                "id": "1199",
                ...
                "avatar": "taikyoku.gif",
                "glops": "40",
                "registerDate": "2001-09-25 01:48:39",
            },
...
class TestFetchPouetData(TestCase):
    def add_groups_response(self):
        ...
        responses.add(
            responses.GET,
            "https://data.pouet.net/dumps/202110/pouetdatadump-groups-20211027.json.gz",
            body=group_gzdata,
        )
    ...
    def add_prods_response(self, prod):
        ...
        responses.add(
            responses.GET, "https://data.pouet.net/dumps/202110/pouetdatadump-prods-20211027.json.gz", body=prod_gzdata
        )
    ...
    def test_run(self):
        ...
        self.assertEqual(astral_blur_pouet.groups.first(), Group.objects.get(name="The Black Lotus"))
        self.assertEqual(astral_blur.links.get(link_class="PouetProduction").parameter, "1")
        self.assertEqual(astral_blur_pouet.download_links.get().url, "https://www.youtube.com/watch?v=eZyLSHyUGBY")
        self.assertEqual(str(astral_blur_pouet.release_date), "March 1997")

        astral_blur_pouet.download_links.create(url="http://example.com/astralblur.mp4", link_type="video")
    ...
    def test_run_with_null_compo_in_placing_data(self):
        ...
        astral_blur_pouet = PouetProduction.objects.get(name="Astral Blur")
        self.assertIsNone(astral_blur_pouet.competition_placings.first().competition_type)
    ...
    def test_nullify_ranking_in_placing_data(self):
        ...
        compo_type, created = CompetitionType.objects.get_or_create(pouet_id=95, defaults={"name": "pc demo"})

        the_gathering = Party.objects.create(name="The Gathering", pouet_id=43)
        astral_blur_pouet.competition_placings.create(
            production=astral_blur_pouet, party=the_gathering, competition_type=compo_type, ranking=3, year=1997
        )
        self.assertEqual(astral_blur_pouet.competition_placings.count(), 1)
        self.assertEqual(astral_blur_pouet.competition_placings.first().ranking, 3)
        ...
        self.assertEqual(astral_blur_pouet.competition_placings.count(), 1)
        self.assertIsNone(astral_blur_pouet.competition_placings.first().ranking)
    ...
    def test_keep_null_ranking_in_placing_data(self):
        ...
        the_gathering = Party.objects.create(name="The Gathering", pouet_id=43)
        astral_blur_pouet.competition_placings.create(
            production=astral_blur_pouet, party=the_gathering, competition_type=compo_type, ranking=None, year=1997
        )
        self.assertEqual(astral_blur_pouet.competition_placings.count(), 1)
        self.assertIsNone(astral_blur_pouet.competition_placings.first().ranking)

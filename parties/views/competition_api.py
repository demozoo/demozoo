...
def handle_production(prod_data, competition):
    if prod_data.get("id"):
        ...
        production = Production(
            release_date=competition.shown_date, updated_at=datetime.datetime.now(), has_bonafide_edits=False
        )
        ...
        if "byline" in prod_data:
            try:
                production.author_nicks.set(
                    [NickSelection(author["id"], author["name"]).commit() for author in prod_data["byline"]["authors"]]
                )
...
def add_placing(request, competition_id):
    ...
    if request.method == "POST":
        ...
        Edit.objects.create(
            ...
            focus=competition,
            focus2=production,
            description=("Added competition placing for %s in %s competition" % (production.title, competition)),
            user=request.user,
        )
...
def update_placing(request, placing_id):
    ...
    if request.method == "POST":
        ...
        if int(data["position"]) > placing.position:  # increasing position - move others down
            competition.placings.filter(position__gt=placing.position, position__lte=data["position"]).update(
                position=F("position") - 1
            )
        elif int(data["position"]) < placing.position:  # decreasing position - move others up
            competition.placings.filter(position__gte=data["position"], position__lt=placing.position).update(
                position=F("position") + 1
            )
        ...
        Edit.objects.create(
            action_type="update_competition_placing",
            focus=competition,
            focus2=production,
            description=("Updated competition placing info for %s in %s competition" % (production.title, competition)),
            user=request.user,
        )
...
def delete_placing(request, placing_id):
    ...
    if request.method == "POST":
        ...
        Edit.objects.create(
            action_type="remove_competition_placing",
            focus=competition,
            focus2=placing.production,
            description=(
                "Removed competition placing for %s in %s competition" % (placing.production.title, competition)
            ),
            user=request.user,
        )

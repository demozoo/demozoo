            ...
            json.dumps(
                {
                    ...
                    "production": {
                        ...
                        "platform_id": zx_spectrum.id,
                        "production_type_id": one_k_intro.id,
                    },
                    "ranking": "1",
                    "score": "",
                }
...
class TestUpdatePlacing(TestCase):
    ...
    def setUp(self):
        ...
        self.mathricks_placing = self.competition.placings.create(position=2, production=self.mathricks, ranking="3")
...
class TestDeletePlacing(TestCase):
    ...
    def setUp(self):
        ...
        self.madrielle = Production.objects.get(title="Madrielle")
        self.madrielle_placing = self.competition.placings.get(production=self.madrielle)
        self.mathricks = Production.objects.create(title="Mathricks", supertype="production", has_bonafide_edits=False)
        self.mathricks_placing = self.competition.placings.create(position=2, production=self.mathricks, ranking="3")

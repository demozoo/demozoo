...
class TestActions(TestCase):
    ...
    def test_fetch_content_length_too_big(self):
        link = self.pondlife.links.create(
            link_class="BaseUrl", parameter="http://example.com/pretend-big-file.txt", is_download_link=True
        )
    ...
    def test_fetch_actual_length_too_big(self):
        ...
        self.assertTrue(
            Download.objects.filter(parameter="http://example.com/real-big-file.txt", error_type="FileTooBig").exists()
        )

    @patch("boto3.Session")
    def test_fetch_from_mirror(self, Session):
        link = self.pondlife.links.create(
            link_class="BaseUrl", parameter="http://example.com/pondlife.txt", is_download_link=True
        )
    ...
    def test_fetch_duplicate_of_existing_file(self):
        link = self.pondlife.links.create(
            link_class="BaseUrl", parameter="http://example.com/pondlife2.txt", is_download_link=True
        )
        pondlife3_download = Download.objects.create(
            link_class="BaseUrl",
            parameter="http://example.com/pondlife3.txt",
            downloaded_at=datetime.datetime(2020, 1, 1, 12, 0, 0),
            mirror_s3_key="1/2/pondlife.123.txt",

...
class TestPouet(TestCase):
    ...
    def setUp(self):
        ...
        ReleaserExternalLink.objects.create(releaser=gasman, link_class="SceneidAccount", parameter="2260")
        raww_arse = Releaser.objects.get(name="Raww Arse")
        ReleaserExternalLink.objects.create(releaser=raww_arse, link_class="PouetGroup", parameter="123")
        forever2e3 = Party.objects.get(name="Forever 2e3")
        PartyExternalLink.objects.create(party=forever2e3, link_class="PouetParty", parameter="181/2000")
    ...
    def test_prod_demozoo_ids(self):
        ...
        response_data = json.loads(response.content)
        results = [result for result in response_data if result["demozoo_id"] == 4 and result["pouet_id"] == 2611]
    ...
    def test_group_demozoo_ids(self):
        ...
        self.assertEqual(response.status_code, 200)
        response_data = json.loads(response.content)
        results = [result for result in response_data if result["demozoo_id"] == 2 and result["pouet_id"] == 123]
        self.assertEqual(1, len(results))
...
class TestZxdemo(TestCase):
    ...
    def setUp(self):
        ...
        raww_arse = Releaser.objects.get(name="Raww Arse")
        ReleaserExternalLink.objects.create(releaser=raww_arse, link_class="ZxdemoAuthor", parameter="404")
        forever2e3 = Party.objects.get(name="Forever 2e3")
        PartyExternalLink.objects.create(party=forever2e3, link_class="ZxdemoParty", parameter="16")
    ...
    def test_prod_demozoo_ids(self):
        ...
        response_data = json.loads(response.content)
        results = [result for result in response_data if result["demozoo_id"] == 4 and result["zxdemo_id"] == 3130]
        self.assertEqual(1, len(results))
    ...
    def test_group_demozoo_ids(self):
        ...
        self.assertEqual(response.status_code, 200)
        response_data = json.loads(response.content)
        results = [result for result in response_data if result["demozoo_id"] == 2 and result["zxdemo_id"] == 404]
        self.assertEqual(1, len(results))
    ...
    def test_party_demozoo_ids(self):
        ...
        response_data = json.loads(response.content)
        results = [result for result in response_data if result["demozoo_id"] == 1 and result["zxdemo_id"] == 16]
        self.assertEqual(1, len(results))
...
class TestKlubi(TestCase):
    ...
    def setUp(self):
        ...
        stevie_dotman = Production.objects.create(
            title="Stevie Dotman", supertype="production", release_date_date="2000-03-18", release_date_precision="d"
        )
        stevie_dotman.types.add(game)

        Production.objects.create(
            title="Mystery Prod", supertype="production", release_date_date="2000-03-18", release_date_precision="d"
        )
...
class TestScenesat(TestCase):
    ...
    def setUp(self):
        Production.objects.create(
            title="Cybernoid's Revenge", supertype="music", release_date_date="2001-03-18", release_date_precision="d"
        )
...
class TestMeteoriks(TestCase):
    ...
    def setUp(self):
        ...
        the_loop = Production.objects.create(
            title="The Loop", supertype="production", release_date_date="2001-03-18", release_date_precision="d"
        )

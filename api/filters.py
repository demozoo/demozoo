...
class ProductionFilter(filters.FilterSet):
    ...
    released_before = filters.DateFilter(field_name="release_date_date", lookup_expr="lt", label="Released before")
    released_since = filters.DateFilter(field_name="release_date_date", lookup_expr="gte", label="Released since")
    added_before = filters.DateTimeFilter(field_name="created_at", lookup_expr="lt", label="Added before")
    added_since = filters.DateTimeFilter(field_name="created_at", lookup_expr="gte", label="Added since")
    updated_before = filters.DateTimeFilter(field_name="updated_at", lookup_expr="lt", label="Updated before")
    updated_since = filters.DateTimeFilter(field_name="updated_at", lookup_expr="gte", label="Updated since")
...
class PartyFilter(filters.FilterSet):
    name = filters.CharFilter(lookup_expr="iexact")
    start_date_before = filters.DateFilter(field_name="start_date_date", lookup_expr="lt", label="Start date before")
    start_date_since = filters.DateFilter(field_name="start_date_date", lookup_expr="gte", label="Start date since")
    country_code = filters.CharFilter(lookup_expr="iexact")

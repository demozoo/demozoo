...
class TestModels(TestCase):
    ...
    def test_file_str(self):
        root_dir = Directory.objects.create(path='/', last_seen_at=datetime.datetime(2020, 1, 1))
        uploading_txt = File.objects.create(
            path='/uploading.txt', directory=root_dir, last_seen_at=datetime.datetime(2020, 1, 1)
        )
    ...
    def test_fetch_oversized_file(self):
        root_dir = Directory.objects.create(path='/', last_seen_at=datetime.datetime(2020, 1, 1))
        bigfile_txt = File.objects.create(
            path='/bigfile.txt', directory=root_dir, last_seen_at=datetime.datetime(2020, 1, 1)
        )
        with self.assertRaises(FileTooBig):
            bigfile_txt.fetched_data()

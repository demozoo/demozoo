...
class TestTasks(TestCase):
    ...
    def test_fetch_new_sceneorg_files(self, sleep):
        ...
        self.assertTrue(File.objects.filter(path="/demos/artists/lazarus/lazarus-taxi-247-compo-version.diz").exists())
    ...
    def test_scan_dir_listing(self, parse_all_dirs):
        ...
        Directory.objects.create(path="/demos/", parent=root_dir, last_seen_at=datetime.datetime(2020, 1, 1))
        warez_dir = Directory.objects.create(
            path="/warez/", parent=root_dir, last_seen_at=datetime.datetime(2020, 1, 1)
        )
        Directory.objects.create(path="/warez/games/", parent=warez_dir, last_seen_at=datetime.datetime(2020, 1, 1))

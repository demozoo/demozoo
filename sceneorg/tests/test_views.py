...
class TestCompoFolderUnLink(TestCase):
    ...
    def test_post(self):
        ...
        response = self.client.post(
            "/sceneorg/compofolders/unlink/", {"directory_id": directory.id, "competition_id": zx1k.id}
        )
        self.assertEqual(response.status_code, 200)
...
class TestCompoFileLink(TestCase):
    ...
    def test_post(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            madrielle.download_links.filter(
...
class TestCompoFileUnlink(TestCase):
    ...
    def test_post(self):
        ...
        madrielle.links.create(
            is_download_link=True, link_class="SceneOrgFile", parameter="/parties/2000/forever00/zx_1k/madrielle.zip"
        )
        madrielle_file = File.objects.get(path="/parties/2000/forever00/zx_1k/madrielle.zip")

        response = self.client.post(
            "/sceneorg/compofiles/unlink/", {"file_id": madrielle_file.id, "production_id": madrielle.id}
        )
        self.assertEqual(response.status_code, 200)
        self.assertFalse(
            madrielle.download_links.filter(

...
def compofolders(request):
    ...
    if request.GET.get("order") == "series":
        ...
        return render(
            request,
            "sceneorg/compofolders/parties_by_series.html",
            {
                "parties": parties,
            },
        )
        ...
        parties = parties.extra(
            select={"date_year": "DATE_TRUNC('year', start_date_date)"}, order_by=["date_year", "name"]
        )
        return render(
            request,
            "sceneorg/compofolders/parties_by_year.html",
            {
                "parties": parties,
            },
        )
...
def compofolder_party(request, party_id):
    ...
    return render(
        request,
        "sceneorg/compofolders/party.html",
        {
            "party": party,
...
def compofolders_show_directory(request, directory_id):
    ...
    return render(
        request,
        "sceneorg/compofolders/show_directory.html",
        {
...
def compofolders_show_competition(request, competition_id):
    ...
    return render(
        request,
        "sceneorg/compofolders/show_competition.html",
        {
            "placings": placings,
        },
    )
...
def compofiles(request):
    ...
    return render(
        request,
        "sceneorg/compofiles/index.html",
        {
...
def compofile_directory(request, directory_id):
    ...
    matches = [(f, Production.objects.get(id=f.production_id)) for f in files if f.production_id is not None]
    ...
    return render(
        request,
        "sceneorg/compofiles/directory.html",
        {
            "directory": directory,
...
def compofile_link(request):
    ...
    if created:
        Edit.objects.create(
            action_type="add_download_link",
            focus=production,
            description=("Added download link %s" % link.url),
            user=request.user,
        )
...
def compofile_unlink(request):
    ...
    links = ProductionLink.objects.filter(
        link_class="SceneOrgFile", parameter=sceneorg_file.path, production_id=production.id
    )
    if links:
        Edit.objects.create(
            action_type="delete_download_link",
            focus=production,
            description=("Deleted download link %s" % links[0].url),
            user=request.user,
        )
        links.delete()

...
class BylineLookup:
    def __init__(
        ...
        if author_matched_nick_fields is None:
            author_matched_nick_fields = [
                MatchedNickField(nick_search, required=False) for nick_search in byline_search.author_nick_searches
            ]

        if affiliation_matched_nick_fields is None:
            affiliation_matched_nick_fields = [
                MatchedNickField(nick_search, required=False) for nick_search in byline_search.affiliation_nick_searches
            ]
    ...
    def render_match_fields(self, name, attrs={}, renderer=None):
        ...
        for i, field in enumerate(self.author_matched_nick_fields):
            ...
            try:
                html = field.widget.render(field_name, self.author_nick_selections[i], attrs=attrs, renderer=renderer)
            except IndexError:
...
class BylineWidget(forms.Widget):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        search_html_output = [
            ...
            self.lookup_widget.render(name + "_lookup", None, attrs=lookup_attrs, renderer=renderer),
        ]
        ...
        output = [
            '<div class="byline_search">' + "".join(search_html_output) + "</div>",
...
class BylineField(forms.Field):
    ...
    def clean(self, value):
        if not value:
            ...
            return Byline(clean_author_nick_selections, clean_affiliation_nick_selections)

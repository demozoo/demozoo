...
class ProductionsFormsTests(TestCase):
    def test_productionindexfilterform_type(self):
        ...
        self.assertEqual(form.fields["production_type"].queryset.count(), ProductionType.featured_types().count())
    ...
    def test_external_links_formset_delete(self):
        ...
        formset = ProductionExternalLinkFormSet(
            data, instance=production, queryset=production.links.filter(is_download_link=False)
        )
...
class TestProductionField(TestCase):
    ...
    def test_submit_with_production_type(self):
        ...
        form = ProdForm(
            {"production_id": "", "production_title": "Froob", "production_byline_search": "", "production_type": ""}
        )

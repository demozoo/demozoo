...
class TestCowbell(TestCase):
    ...
    def test_modland_nonstandard(self):
        link = self.cybrev.links.create(
            link_class="ModlandFile", parameter="/pub/modules/OctaMED MMD0/Laxical/smoked.mmd0", is_download_link=True
        )
        filetype, url = identify_link_as_track(link)
        self.assertEqual(filetype, "openmpt")
    ...
    def test_modland_psid(self):
        link = self.cybrev.links.create(
            link_class="ModlandFile", parameter="/pub/modules/PlaySID/Vincenzo/habvero.psid", is_download_link=True
        )
        filetype, url = identify_link_as_track(link)
    ...
    def test_modland_sid(self):
        link = self.cybrev.links.create(
            link_class="ModlandFile", parameter="/pub/modules/PlaySID/Vincenzo/habvero.sid", is_download_link=True
        )
        filetype, url = identify_link_as_track(link)
        self.assertEqual(filetype, "sid")
    ...
    def test_media_demozoo(self):
        link = self.cybrev.links.create(
            link_class="BaseUrl", parameter="https://media.demozoo.org/music/21/3d/zap.sid", is_download_link=True
        )
        filetype, url = identify_link_as_track(link)
        self.assertEqual(filetype, "sid")
    ...
    def test_modarchive(self):
        link = self.cybrev.links.create(link_class="ModarchiveModule", parameter="123", is_download_link=False)
        filetype, url = identify_link_as_track(link)
        self.assertEqual(filetype, "openmpt")

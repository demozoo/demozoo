...
class TestShowProduction(TestCase):
    ...
    def test_get_with_video(self):
        ...
        pondlife.links.update(
            ...
            thumbnail_url="http://example.com/yt.png",
            thumbnail_width=320,
...
class TestEditCoreDetails(TestCase):
    ...
    def test_set_music_type_and_platform(self):
        cybrev = Production.objects.get(title="Cybernoid's Revenge")
        response = self.client.post(
            "/productions/%d/edit_core_details/" % cybrev.id,
            {
    ...
    def test_set_graphics_type_and_platform(self):
        skyrider = Production.objects.get(title="Skyrider")
        response = self.client.post(
            "/productions/%d/edit_core_details/" % skyrider.id,
            {
                "title": "Skyrider",
                "byline_search": "Gasman",
                "byline_author_match_0_id": Nick.objects.get(name="Gasman").id,
...
class TestEditNotes(TestCase):
    ...
    def test_post(self):
        response = self.client.post(
            "/productions/%d/edit_notes/" % self.pondlife.id,
            {
...
class TestAddBlurb(TestCase):
    ...
    def test_post(self):
        response = self.client.post(
            "/productions/%d/add_blurb/" % self.pondlife.id,
            {
...
class TestDeleteBlurb(TestCase):
    ...
    def test_post(self):
        response = self.client.post(
            "/productions/%d/delete_blurb/%d/" % (self.pondlife.id, self.blurb.id), {"yes": "yes"}
        )
        self.assertRedirects(response, "/productions/%d/" % self.pondlife.id)
...
class TestEditExternalLinks(TestCase):
    ...
    def test_post(self):
        ...
        self.assertRedirects(response, "/productions/%d/" % self.pondlife.id)
        self.assertEqual(
            ProductionLink.objects.filter(production=self.pondlife, link_class="PouetProduction").count(), 1
        )
...
class TestEditDownloadLinks(TestCase):
    ...
    def test_post(self):
        response = self.client.post(
            "/productions/%d/edit_download_links/" % self.pondlife.id,
            {
                "links-TOTAL_FORMS": 1,
                "links-INITIAL_FORMS": 0,
                "links-MIN_NUM_FORMS": 0,
        ...
        self.assertRedirects(response, "/productions/%d/" % self.pondlife.id)
        self.assertEqual(ProductionLink.objects.filter(production=self.pondlife, link_class="SceneOrgFile").count(), 1)
...
class TestAddScreenshot(TestCase):
    ...
    def test_post_production_single(self, create_screenshot_versions_from_local_file):
        pondlife = Production.objects.get(title="Pondlife")
        response = self.client.post(
            "/productions/%d/add_screenshot/" % pondlife.id,
            {
    ...
    def test_post_music_single(self, create_screenshot_versions_from_local_file):
        cybrev = Production.objects.get(title="Cybernoid's Revenge")
        response = self.client.post(
            "/productions/%d/add_artwork/" % cybrev.id,
            {
                "screenshot": SimpleUploadedFile("test.png", get_test_image().file.getvalue()),
            },
        )
...
class TestDeleteScreenshot(TestCase):
    ...
    def test_post_production(self):
        response = self.client.post(
            "/productions/%d/delete_screenshot/%d/" % (self.pondlife.id, self.pondlife_screenshot.id), {"yes": "yes"}
        )
    ...
    def test_post_music(self):
        response = self.client.post(
            "/productions/%d/delete_artwork/%d/" % (self.cybrev.id, self.cybrev_artwork.id), {"yes": "yes"}
        )
...
class TestEditPackContents(TestCase):
    ...
    def test_post(self):
        response = self.client.post(
            "/productions/%d/edit_pack_contents/" % self.pondlife.id,
            {
                "form-TOTAL_FORMS": 3,
                "form-INITIAL_FORMS": 1,
                "form-MIN_NUM_FORMS": 0,
...
class TestEditTags(TestCase):
    ...
    def test_post(self):
        response = self.client.post("/productions/%d/edit_tags/" % self.pondlife.id, {"tags": "fish, ducks"})
        self.assertRedirects(response, "/productions/%d/" % self.pondlife.id)
        self.assertEqual(self.pondlife.tags.count(), 2)

    def test_tag_replacement(self):
        response = self.client.post("/productions/%d/edit_tags/" % self.pondlife.id, {"tags": "fish, 48k"})
        self.assertRedirects(response, "/productions/%d/" % self.pondlife.id)
        self.assertEqual(self.pondlife.tags.count(), 2)
        self.assertTrue(self.pondlife.tags.filter(name="zx-spectrum-48k").exists())

    def test_post_locked(self):
        response = self.client.post("/productions/%d/edit_tags/" % self.mooncheese.id, {"tags": "wensleydale, stilton"})
        self.assertEqual(response.status_code, 403)
...
class TestAddTag(TestCase):
    ...
    def test_post(self):
        response = self.client.post("/productions/%d/add_tag/" % self.pondlife.id, {"tag_name": "fish"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(self.pondlife.tags.count(), 1)

    def test_post_blacklisted(self):
        response = self.client.post("/productions/%d/add_tag/" % self.pondlife.id, {"tag_name": "demo"})
        self.assertEqual(response.status_code, 200)
    ...
    def test_post_locked(self):
        response = self.client.post("/productions/%d/add_tag/" % self.mooncheese.id, {"tag_name": "wensleydale"})
        self.assertEqual(response.status_code, 403)
        self.assertEqual(self.mooncheese.tags.count(), 0)
...
class TestRemoveTag(TestCase):
    ...
    def test_post(self):
        response = self.client.post("/productions/%d/remove_tag/" % self.pondlife.id, {"tag_name": "fish"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(self.pondlife.tags.count(), 0)

    def test_post_locked(self):
        response = self.client.post("/productions/%d/remove_tag/" % self.mooncheese.id, {"tag_name": "wensleydale"})
        self.assertEqual(response.status_code, 403)
...
class TestAutocomplete(TestCase):
    ...
    def test_get_with_url(self):
        response = self.client.get(
            "/productions/autocomplete/", {"term": "https://demozoo.org/music/5/", "supertype": "music"}
        )
        self.assertEqual(response.status_code, 200)
        response_json = json.loads(response.content)
        self.assertEqual(response_json[0]["title"], "Cybernoid's Revenge")
...
class TestDelete(TestCase):
    ...
    def test_post(self):
        response = self.client.post("/productions/%d/delete/" % self.pondlife.id, {"yes": "yes"})
        self.assertRedirects(response, "/productions/")
        self.assertFalse(Production.objects.filter(title="Pondlife").exists())

    def test_cancel(self):
        response = self.client.post("/productions/%d/delete/" % self.pondlife.id, {"no": "no"})
        self.assertRedirects(response, "/productions/%d/" % self.pondlife.id)
...
class TestLocking(TestCase):
    ...
    def test_post_lock(self):
        self.client.login(username="testsuperuser", password="12345")
        response = self.client.post("/productions/%d/lock/" % self.pondlife.id, {"yes": "yes"})
        self.assertRedirects(response, "/productions/%d/" % self.pondlife.id)
    ...
    def test_post_unlock(self):
        self.client.login(username="testsuperuser", password="12345")
        response = self.client.post("/productions/%d/protected/" % self.mooncheese.id, {"yes": "yes"})
    ...
    def test_non_superuser_cannot_unlock(self):
        self.client.login(username="testuser", password="12345")
        self.client.post("/productions/%d/protected/" % self.mooncheese.id, {"yes": "yes"})
...
class TestEditInfoFiles(MediaTestMixin, TestCase):
    ...
    def test_add_one(self):
        self.client.login(username="testuser", password="12345")
        response = self.client.post(
            "/productions/%d/edit_info/" % self.pondlife.id,

...
class TestTasks(TestCase):
    ...
    def test_fetch_production_link_embed_data_fetched_last_month(self):
        ...
        link = pondlife.links.create(link_class="SpeccyWikiPage", parameter="ldoVS0idTBw", is_download_link=False)
    ...
    def test_fetch_production_link_embed_data_error(self):
        pondlife = Production.objects.get(title="Pondlife")
        link = pondlife.links.create(link_class="SpeccyWikiPage", parameter="404", is_download_link=False)
        ProductionLink.objects.filter(id=link.id).update(link_class="YoutubeVideo")
        fetch_production_link_embed_data(link.id)
        link.refresh_from_db()

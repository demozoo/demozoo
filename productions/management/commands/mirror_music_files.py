...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        filetype_filter = (
            Q(parameter__iendswith=".sap")
            | Q(parameter__iendswith=".sid")
            | Q(parameter__iendswith=".sndh")
            | Q(parameter__iendswith=".mod")
            | Q(parameter__iendswith=".s3m")
            | Q(parameter__iendswith=".xm")
            | Q(parameter__iendswith=".it")
        )
        links = (
        ...
        for prod_link in links:
            ...
            try:
                ...
                ProductionLink.objects.create(
                    production=prod_link.production, link_class="BaseUrl", parameter=new_url, is_download_link=True
                )

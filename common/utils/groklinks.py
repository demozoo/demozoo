    ...
    def __init__(
        ...
        if allowed_hostnames is not None or url is None:
            ...
            is_website = self.allowed_schemes is not None and (
                "http" in self.allowed_schemes or "https" in self.allowed_schemes
            )
            if is_website and parsed_url.hostname.startswith("www."):
                self.allowed_hostnames = [parsed_url.hostname, parsed_url.hostname.replace("www.", "", 1)]
            elif is_website:
                self.allowed_hostnames = [parsed_url.hostname, "www." + parsed_url.hostname]
            else:
                self.allowed_hostnames = [parsed_url.hostname]

    def matches_url(self, url):
        return (self.allowed_schemes is None or url.scheme in self.allowed_schemes) and (
...
pouet = Site("Pouët", classname="pouet", url="https://www.pouet.net/")
...
csdb = Site("CSDb", url="https://csdb.dk/", allowed_hostnames=["csdb.dk", "www.csdb.dk", "noname.c64.org"])
...
nectarine = Site(
    "Nectarine",
    long_name="Nectarine Demoscene Radio",
    url="https://scenestream.net/",
...
artcity = Site(
    "ArtCity",
    url="http://artcity.bitfellas.org/",
    allowed_hostnames=["artcity.bitfellas.org"],
...
class SceneOrgFile(AbstractBaseUrl):
    ...
    @property
    def info_url(self):
        return "https://files.scene.org/view%s" % urllib.parse.quote(self.param.encode("iso-8859-1"))

    @property
    def auto_mirror_url(self):
        return "https://files.scene.org/get%s" % urllib.parse.quote(self.param.encode("iso-8859-1"))
    ...
    @property
    def nl_url(self):
        return "ftp://ftp.scene.org/pub%s" % urllib.parse.quote(self.param.encode("iso-8859-1"))

    @property
    def nl_http_url(self):
        return "http://archive.scene.org/pub%s" % urllib.parse.quote(self.param.encode("iso-8859-1"))

    @property
    def nl_https_url(self):
        return "https://archive.scene.org/pub%s" % urllib.parse.quote(self.param.encode("iso-8859-1"))
...
class UntergrundFile(AbstractBaseUrl):
    site = Site(
        "untergrund.net",
        classname="untergrund",
        url="https://ftp.untergrund.net/",
        allowed_schemes=["https", "ftp"],
        allowed_hostnames=["ftp.untergrund.net", "untergrund.net"],
    )
...
class PouetParty(AbstractBaseUrl):
    ...
    def __str__(self):
        ...
        return "https://www.pouet.net/party.php?which=%s&when=%s" % (id, year)
...
class SceneOrgFolder(AbstractBaseUrl):
    ...
    def __str__(self):
        return "https://files.scene.org/browse%s" % urllib.parse.quote(self.param.encode("iso-8859-1"))
...
class YoutubeVideo(AbstractBaseUrl):
    ...
    def __str__(self):
        if "/" in self.param:
            ...
            return "https://www.youtube.com/watch?v=%s&t=%s" % (id, timestamp)
        else:
            return "https://www.youtube.com/watch?v=%s" % self.param
...
c64ch = Site(
    "C64.CH",
    url="https://c64.ch/",
    allowed_hostnames=["c64.ch", "www.c64.ch"],
...
class DiscogsRelease(AbstractBaseUrl):
    ...
    def __str__(self):
        ...
        return "http://www.discogs.com/%s/release/%s" % (slug, id)
...
class ModarchiveMember(AbstractBaseUrl):
    ...
    tests = [
        ...
        querystring_match(
            r"https?://(?:www\.)?modarchive\.org/index\.php", "query", othervars={"request": "view_profile"}
        ),
    ]
...
class ModarchiveModule(AbstractBaseUrl):
    ...
    tests = [
        ...
        querystring_match(
            r"https?://(?:www\.|lite\.)?modarchive\.org/index\.php", "query", othervars={"request": "view_by_moduleid"}
        ),
...
hall_of_light = Site(
    "Hall Of Light",
    classname="hall_of_light",
    url="http://hol.abime.net/",
    allowed_hostnames=["hol.abime.net"],
...
spotify = Site("Spotify", url="https://play.spotify.com/", allowed_hostnames=["open.spotify.com", "play.spotify.com"])
...
class GithubDirectory(AbstractBaseUrl):
    ...
    def __str__(self):
        ...
        dirs = "/".join(params[2:])
        return "http://github.com/%s/%s/tree/%s" % (user, repo, dirs)
...
class InternetArchivePage(AbstractBaseUrl):
    site = Site(
        "Internet Archive",
        long_name="the Internet Archive",
        classname="internetarchive",
        url="https://archive.org/",
        icon_path="images/icons/external_sites/archive_org.png",
    )
...
class WaybackMachinePage(AbstractBaseUrl):
    site = Site(
        "Wayback Machine",
        long_name="the Wayback Machine",
        classname="waybackmachine",
        url="https://web.archive.org/",
        icon_path="images/icons/external_sites/wayback.png",
    )
...
gameboy_demospotting = Site(
    ...
    classname="gameboydemospotting",
    url="http://gameboy.modermodemet.se/",
...
plus4world = Site(
    "Plus/4 World",
    classname="plus4world",
    url="http://plus4world.powweb.com/",
...
class BandcampTrack(AbstractBaseUrl):
    ...
    def __str__(self):
        ...
        return "https://%s.bandcamp.com/track/%s" % (domain, name)


class TwitchChannel(UrlPattern):
    site = Site("Twitch", url="https://twitch.tv/", allowed_hostnames=["m.twitch.tv", "twitch.tv", "www.twitch.tv"])
    pattern = "/<slug>"
...
lexaloffle = Site("Pico-8", url="https://www.lexaloffle.com/", long_name="Lexaloffle", classname="pico8")
...
class DOPEdition(AbstractBaseUrl):
    site = Site(
        "DOP",
        long_name="The Diskmag Onlinification Project",
    ...
    def __str__(self):
        ...
        return "https://diskmag.conspiracy.hu/magazine/%s/#/edition=%s" % (magazine, edition)
...
class EventsRetrosceneRelease(AbstractBaseUrl):
    ...
    tests = [
        path_regex_match(r"/((?!files)[\w-]+/[\w-]+/\d+)"),
    ]
...
class SpectrumComputingRelease(AbstractBaseUrl):
    ...
    tests = [
        ...
        querystring_match(r"https?://(?:www\.)?spectrumcomputing\.co\.uk/index\.php", "id", othervars={"cat": "96"}),
    ]
...
PRODUCTION_LINK_TYPES = [
    ...
    PushnpopProduction,
    SpotifyTrack,
    Plus4WorldProduction,
    SpeccyPlProduction,
    AtarikiEntry,
    SixteenColorsPack,
    ShadertoyShader,
    AmigascneFile,
    PaduaOrgFile,  # sites mirrored by scene.org - must come before SceneOrgFile
    SceneOrgFile,
    FujiologyFile,
    UntergrundFile,
    GithubAccount,
...
PARTY_SERIES_LINK_TYPES = [
    ...
    AtarikiEntry,
    InstagramAccount,
    LivecodeDemozooEvent,
    TikTokUser,
    WaybackMachinePage,
    DemopartyNetPartySeries,
    BaseUrl,
]
...
TOURNAMENT_LINK_TYPES = [YoutubeVideo, VimeoVideo, ShadertoyShader, Tic80Cart, BaseUrl]

...
def home(request):
    ...
    latest_additions = (
        Production.objects.filter(platforms__id__in=ZXDEMO_PLATFORM_IDS)
        .order_by("-created_at")
        .prefetch_related("author_nicks__releaser", "author_affiliation_nicks__releaser")[:10]
    )
...
def author(request, releaser_id):
    ...
    if releaser.is_group:
        member_memberships = filter_releasers_queryset_to_spectrum(
            releaser.member_memberships.select_related("member"), releaser_table="T3"
        )
    else:
        member_memberships = Membership.objects.none()

    group_memberships = filter_releasers_queryset_to_spectrum(
        releaser.group_memberships.select_related("group"), releaser_table="T3"
    )
...
def parties(request):
    ...
    parties_with_spectrum_entries = list(
        Party.objects.filter(competitions__placings__production__platforms__id__in=ZXDEMO_PLATFORM_IDS)
        .distinct()
        .values_list("id", flat=True)
    )
    parties_with_spectrum_invitations = list(
        Party.objects.filter(invitations__platforms__id__in=ZXDEMO_PLATFORM_IDS).distinct().values_list("id", flat=True)
    )

    parties = Party.objects.filter(id__in=(parties_with_spectrum_entries + parties_with_spectrum_invitations)).order_by(
        "start_date_date"
    )
    return render(
        request,
...
def parties_year(request, year):
    ...
    parties = (
        Party.objects.filter(
            id__in=(parties_with_spectrum_entries + parties_with_spectrum_invitations), start_date_date__year=year
        )
        .distinct()
...
def party(request, party_id):
    ...
    competitions = (
        party.competitions.filter(placings__production__platforms__id__in=ZXDEMO_PLATFORM_IDS)
        .distinct()
        .order_by("name", "id")
    )
    ...
    for competition in competitions:
        ...
        screenshots = Screenshot.objects.filter(
            production__competition_placings__competition=competition, production__platforms__id__in=ZXDEMO_PLATFORM_IDS
        ).order_by("?")[: math.ceil(len(placings) / 6.0)]
    ...
    invitations = party.invitations.filter(platforms__id__in=ZXDEMO_PLATFORM_IDS).prefetch_related(
...
def search(request):
    ...
    demos = (
        Production.objects.filter(
            platforms__id__in=ZXDEMO_PLATFORM_IDS, supertype="production", title__icontains=search_term
        )
        .order_by("sortable_title")
        .prefetch_related("links", "screenshots", "author_nicks", "author_affiliation_nicks")
    )
    ...
    if demoskip > 0:
        url_params = base_url_params.copy()
        url_params.update({"feature": "demos", "demoskip": max(0, demoskip - 10)})
    ...
    musics = (
        Production.objects.filter(
            platforms__id__in=ZXDEMO_PLATFORM_IDS, supertype="music", title__icontains=search_term
        )
        .order_by("sortable_title")
        .prefetch_related("links", "screenshots", "author_nicks", "author_affiliation_nicks")
    )
    ...
    if musicskip > 0:
        url_params = base_url_params.copy()
        url_params.update({"feature": "music", "musicskip": max(0, musicskip - 10)})
    ...
    graphics = (
        Production.objects.filter(
            platforms__id__in=ZXDEMO_PLATFORM_IDS, supertype="graphics", title__icontains=search_term
        )
        .order_by("sortable_title")
        .prefetch_related("links", "screenshots", "author_nicks", "author_affiliation_nicks")
    )
    ...
    if gfxskip > 0:
        url_params = base_url_params.copy()
        url_params.update({"feature": "graphics", "gfxskip": max(0, gfxskip - 10)})
    ...
    sceners = (
        spectrum_releasers()
        .filter(nicks__name__icontains=search_term)
        .distinct()
        .extra(select={"lower_name": "lower(demoscene_releaser.name)"})
        .order_by("lower_name")
    )
    ...
    if len(sceners) == 11:
        sceners = sceners[:10]
        url_params = base_url_params.copy()
        url_params.update({"feature": "sceners", "scenerskip": scenerskip + 10})
    ...
    if scenerskip > 0:
        url_params = base_url_params.copy()
        url_params.update({"feature": "sceners", "scenerskip": max(0, scenerskip - 10)})

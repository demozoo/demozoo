    ...
    def search(self, page_number=1, count=50):
        ...
        if "platform" in filter_expressions or "on" in filter_expressions:
            ...
            for platform_name in platforms:
                platform_ids |= Platform.objects.filter(
                    Q(name__iexact=platform_name) | Q(aliases__name__iexact=platform_name)
        ...
        if "year" in filter_expressions or "date" in filter_expressions:
            ...
            for date_str in filter_expressions["year"] | filter_expressions["date"]:
                ...
                party_filter_q &= Q(
                    end_date_date__gte=date_expr.date_range_start(), start_date_date__lte=date_expr.date_range_end()
                )
        ...
        if "production" in to_fetch:
            ...
            if has_search_term:
                productions = productions.annotate(search_snippet=TSHeadline("notes", psql_query))
        ...
        if "releaser" in to_fetch:
            ...
            if has_search_term:
                releasers = releasers.annotate(search_snippet=TSHeadline("notes", psql_query))
        ...
        if "party" in to_fetch:
            ...
            if has_search_term:
                parties = parties.annotate(search_snippet=TSHeadline("notes", psql_query))
        ...
        if "bbs" in to_fetch:
            ...
            if has_search_term:
                bbses = bbses.annotate(search_snippet=TSHeadline("notes", psql_query))
            for bbs in bbses:

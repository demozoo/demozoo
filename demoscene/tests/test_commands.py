...
class TestBumpExternalLinks(TransactionTestCase):
    ...
    def test_run(self):
        ...
        mooncheese.links.create(
            ...
            parameter="/mirrors/amigascne/Groups/A/Abandon/Abandon-NewDemosIntro",
        ...
        self.assertTrue(mooncheese.links.filter(link_class="PouetProduction", parameter="13121").exists())
        self.assertFalse(mooncheese.links.filter(link_class="SceneOrgFile").exists())
        self.assertEqual(mooncheese.links.filter(link_class="AmigascneFile").count(), 1)
...
class TestSanity(MediaTestMixin, TestCase):
    ...
    def test_run(self):
        ...
        invitation_compo = Party.objects.get(name="Forever 2e3").competitions.create(name="Invitation compo")
        mooncheese = Production.objects.get(title="Mooncheese")
        invitation_compo.placings.create(production=mooncheese, ranking=1, position=1)

        invitation_not_compo = Party.objects.get(name="Forever 2e3").competitions.create(name="Invitation not-compo")
        invitation_not_compo.placings.create(production=Production.objects.get(title="Pondlife"), position=1)

        party = Party.objects.get(name="Forever 2e3")
        ResultsFile.objects.create(
            party=party,
            file=File(
                name="forever2e3.txt", file=BytesIO(b"You get a c\xe5r! You get a c\xe5r! Everybody gets a c\xe5r!")
            ),
            encoding="utf-8",
        ...
        results2 = ResultsFile.objects.create(
            party=party,
            file=File(name="forever2e3a.txt", file=BytesIO(b"hello")),
            encoding="utf-8",
            filename="forever2e3.txt",
            filesize=100,
            sha1="1234123412341234",
        ...
        self.assertTrue(Party.objects.get(name="Forever 2e3").competitions.filter(name="Invitation compo").exists())
        self.assertFalse(
            Party.objects.get(name="Forever 2e3").competitions.filter(name="Invitation not-compo").exists()
        )
        self.assertTrue(Party.objects.get(name="Forever 2e3").invitations.filter(title="Mooncheese").exists())
        self.assertTrue(Party.objects.get(name="Forever 2e3").invitations.filter(title="Pondlife").exists())
        ...
        self.assertFalse(Production.objects.get(title="Mooncheese").pack_members.filter(member=coverstory).exists())
        self.assertEqual(Production.objects.get(title="Pondlife").pack_members.filter(member=madrielle).count(), 1)
        self.assertEqual(Production.objects.get(title="Pondlife").pack_members.get(member=mooncheese).position, 2)
        self.assertTrue(Production.objects.get(title="Subliminal Extacy #1").types.filter(name="Pack").exists())
...
class TestFindSceneorgResultsFiles(TestCase):
    ...
    def setUp(self):
        ...
        self.forever.external_links.create(link_class="SceneOrgFolder", parameter="/parties/2000/forever00/")
        sceneorg_dir = Directory.objects.create(
            path="/parties/2000/forever00/",
            last_seen_at=datetime.datetime.now(),
        )
        self.resultsfile = SceneOrgFile.objects.create(
            path="/parties/2000/forever00/results.txt", last_seen_at=datetime.datetime.now(), directory=sceneorg_dir
        )

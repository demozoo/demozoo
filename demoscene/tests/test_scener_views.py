...
class TestEditLocation(TestCase):
    ...
    def test_post(self):
        self.client.login(username="testuser", password="12345")
        response = self.client.post(
            "/sceners/%d/edit_location/" % self.gasman.id,
            {
                "location": "Oxford",
            },
        )
...
class TestEditRealName(TestCase):
    ...
    def test_post(self):
        self.client.login(username="testsuperuser", password="12345")
        response = self.client.post(
            "/sceners/%d/edit_real_name/" % self.gasman.id,
            {
                "first_name": "Matt",
                "surname": "Gustavsson",
    ...
    def test_post_ajax(self):
        self.client.login(username="testsuperuser", password="12345")
        response = self.client.post(
            "/sceners/%d/edit_real_name/?ajax_submit=true" % self.gasman.id,
            {
                "first_name": "Matt",
                "surname": "Gustavsson",
                "real_name_note": "yes really",
...
class TestRemoveGroup(TestCase):
    ...
    def test_ex_member(self):
        response = self.client.post(
            "/sceners/%d/remove_group/%d/" % (self.gasman.id, self.hooy_program.id),
            {
...
class TestConvertToGroup(TestCase):
    ...
    def test_post(self):
        response = self.client.post(
            "/sceners/%d/convert_to_group/" % self.laesq.id,
            {
                "yes": "yes",
            },
        )

...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        ...
        for k, stls in groupby(
            SoundtrackLink.objects.order_by("production_id", "position"), lambda stl: stl.production_id
        ):
        ...
        pack_contents = (
            PackMember.objects.filter(pack__types__in=non_pack_type_ids, member__supertype="music")
            .exclude(pack_id__in=actual_pack_ids)
            .order_by("pack_id", "position")
        )
        for pack_id, pack_members in groupby(pack_contents, lambda pm: pm.pack_id):
            soundtrack_ids = list(
                SoundtrackLink.objects.filter(production_id=pack_id)
                .order_by("position")
            ...
            for pack_member in pack_members:
                if pack_member.member_id not in soundtrack_ids:
                    SoundtrackLink.objects.create(
                        production_id=pack_id,
                        soundtrack_id=pack_member.member_id,
                        position=len(soundtrack_ids) + 1,
                        data_source=pack_member.data_source,
                    )

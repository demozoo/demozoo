...
class NickLookup:
    def __init__(
        ...
        nick_search = NickSearch(search_term, sceners_only=sceners_only, groups_only=groups_only, group_ids=group_ids)
    ...
    def from_value(value, sceners_only=False, groups_only=False, prefer_members_of=None):
        ...
        if not value:
            return NickLookup(sceners_only=sceners_only, groups_only=groups_only, prefer_members_of=prefer_members_of)
...
class NickWidget(forms.Widget):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        search_html_output = [
            self.search_widget.render(name + "_search", nick_lookup.search_term, attrs=attrs, renderer=renderer),
            ...
            self.lookup_widget.render(name + "_lookup", None, attrs=attrs, renderer=renderer),
        ]
        ...
        if self.sceners_only:
            root_classname = "nick_field sceners_only"
        elif self.groups_only:
            root_classname = "nick_field groups_only"
        else:
        ...
        output = [
            ...
            '<div class="nick_match_container">' + matched_nick_html + "</div>",
        ]
        return mark_safe('<div class="' + root_classname + '"' + root_attrs + ">" + "".join(output) + "</div>")

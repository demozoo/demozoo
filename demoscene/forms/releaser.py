...
class CreateGroupForm(forms.ModelForm):
    ...
    def log_creation(self, user):
        Edit.objects.create(
            ...
            focus=self.instance,
            description=("Added group '%s'" % self.instance.name),
            user=user,
        )
...
class CreateScenerForm(forms.ModelForm):
    ...
    def log_creation(self, user):
        Edit.objects.create(
            ...
            focus=self.instance,
            description=("Added scener '%s'" % self.instance.name),
            user=user,
        )
...
class ScenerEditLocationForm(ModelFormWithLocation):
    def log_edit(self, user):
        Edit.objects.create(
            ...
            focus=self.instance,
            description=("Set location to %s" % self.instance.location),
            user=user,
        )
...
class ScenerEditRealNameForm(forms.ModelForm):
    def log_edit(self, user):
        ...
        if "first_name" in changed_fields or "surname" in changed_fields:
            ...
            Edit.objects.create(
                action_type="edit_scener_real_name", focus=self.instance, description="Set real name", user=user
            )
...
class ReleaserEditNotesForm(forms.ModelForm):
    def log_edit(self, user):
        Edit.objects.create(
            action_type="edit_releaser_notes", focus=self.instance, description="Edited notes", user=user
        )
...
class NickForm(forms.ModelForm):
    ...
    def __init__(self, releaser, *args, **kwargs):
        ...
        if not (instance and instance.name == releaser.name):
            self.fields["override_primary_nick"] = forms.BooleanField(
                label="Use this as their preferred name, instead of '%s'" % releaser.name, required=False
            )
    ...
    def validate_unique(self):
        ...
        try:
            ...
            e.error_dict["__all__"] = ["This nick cannot be added, as it duplicates an existing one."]
    ...
    def log_creation(self, user):
        Edit.objects.create(
            ...
            focus=self.instance.releaser,
            description=("Added nick '%s'" % self.instance.name),
            user=user,
        )
...
ReleaserExternalLinkFormSet = inlineformset_factory(
    Releaser, ReleaserExternalLink, form=ReleaserExternalLinkForm, formset=BaseExternalLinkFormSet
)

...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding model 'ReleaserExternalLink'
        db.create_table('demoscene_releaserexternallink', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('link_class', self.gf('django.db.models.fields.CharField')(max_length=100)),
            ('parameter', self.gf('django.db.models.fields.CharField')(max_length=255)),
            ('releaser', self.gf('django.db.models.fields.related.ForeignKey')(related_name='external_links', to=orm['demoscene.Releaser'])),
        ))
        db.send_create_signal('demoscene', ['ReleaserExternalLink'])

        for releaser in orm.Releaser.objects.all():
            if releaser.sceneid_user_id:
                orm.ReleaserExternalLink.objects.create(
                    link_class = 'SceneidAccount', parameter = releaser.sceneid_user_id, releaser = releaser
                )
            if releaser.slengpung_user_id:
                orm.ReleaserExternalLink.objects.create(
                    link_class = 'SlengpungUser', parameter = releaser.slengpung_user_id, releaser = releaser
                )
            if releaser.amp_author_id:
                orm.ReleaserExternalLink.objects.create(
                    link_class = 'AmpAuthor', parameter = releaser.amp_author_id, releaser = releaser
                )
            if releaser.csdb_author_id:
                orm.ReleaserExternalLink.objects.create(
                    link_class = 'CsdbScener', parameter = releaser.csdb_author_id, releaser = releaser
                )
            if releaser.nectarine_author_id:
                orm.ReleaserExternalLink.objects.create(
                    link_class = 'NectarineArtist', parameter = releaser.nectarine_author_id, releaser = releaser
                )
            if releaser.bitjam_author_id:
                orm.ReleaserExternalLink.objects.create(
                    link_class = 'BitjamAuthor', parameter = releaser.bitjam_author_id, releaser = releaser
                )
            if releaser.artcity_author_id:
                orm.ReleaserExternalLink.objects.create(
                    link_class = 'ArtcityArtist', parameter = releaser.artcity_author_id, releaser = releaser
                )
            if releaser.mobygames_author_id:
                orm.ReleaserExternalLink.objects.create(
                    link_class = 'MobygamesDeveloper', parameter = releaser.mobygames_author_id, releaser = releaser
                )
            if releaser.asciiarena_author_id:
                orm.ReleaserExternalLink.objects.create(
                    link_class = 'AsciiarenaArtist', parameter = releaser.asciiarena_author_id, releaser = releaser
                )
            if releaser.zxdemo_id:
                orm.ReleaserExternalLink.objects.create(
                    link_class = 'ZxdemoAuthor', parameter = releaser.zxdemo_id, releaser = releaser
                )

    def backwards(self, orm):
        
        # Deleting model 'ReleaserExternalLink'
        db.delete_table('demoscene_releaserexternallink')

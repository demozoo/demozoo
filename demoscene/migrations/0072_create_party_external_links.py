...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding model 'PartyExternalLink'
        db.create_table('demoscene_partyexternallink', (
            ...
            ('party', self.gf('django.db.models.fields.related.ForeignKey')(related_name='external_links', to=orm['demoscene.Party'])),
        ))
        db.send_create_signal('demoscene', ['PartyExternalLink'])
        
        for party in orm.Party.objects.all():
            if party.bitworld_party_id:
                orm.PartyExternalLink(
                    link_class = 'BitworldParty', parameter = party.bitworld_party_id, party_id = party.id
                ).save()
            if party.breaks_amiga_party_id:
                orm.PartyExternalLink(
                    link_class = 'BreaksAmigaParty', parameter = party.breaks_amiga_party_id, party_id = party.id
                ).save()
            if party.csdb_party_id:
                orm.PartyExternalLink(
                    link_class = 'CsdbEvent', parameter = party.csdb_party_id, party_id = party.id
                ).save()
            if party.demoparty_net_url_fragment:
                orm.PartyExternalLink(
                    link_class = 'DemopartyNetParty', parameter = party.demoparty_net_url_fragment, party_id = party.id
                ).save()
            if party.homepage:
                orm.PartyExternalLink(
                    link_class = 'BaseUrl', parameter = party.homepage, party_id = party.id
                ).save()
            if party.pouet_party_id and party.pouet_party_when:
                orm.PartyExternalLink(
                    link_class = 'PouetParty', parameter = "%s/%s" % (party.pouet_party_id, party.pouet_party_when), party_id = party.id
                ).save()
            if party.scene_org_directory:
                dir = party.scene_org_directory.strip()
                if dir[-1] != '/':
                	dir += '/'
                orm.PartyExternalLink(
                    link_class = 'SceneOrgFolder', parameter = dir, party_id = party.id
                ).save()
            if party.slengpung_party_id:
                orm.PartyExternalLink(
                    link_class = 'SlengpungParty', parameter = party.slengpung_party_id, party_id = party.id
                ).save()
            if party.twitter_username:
                orm.PartyExternalLink(
                    link_class = 'TwitterAccount', parameter = party.twitter_username, party_id = party.id
                ).save()
            if party.zxdemo_party_id:
                orm.PartyExternalLink(
                    link_class = 'ZxdemoParty', parameter = party.zxdemo_party_id, party_id = party.id
                ).save()

    def backwards(self, orm):
        
        # Deleting model 'PartyExternalLink'
        db.delete_table('demoscene_partyexternallink')

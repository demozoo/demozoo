...
def authors(request):
    ...
    authors = (
        AuthorMatchInfo.objects.select_related("releaser")
        .order_by("releaser__name")
...
def production_link(request):
    ...
    if created:
        Edit.objects.create(
            ...
            focus=production,
            description=("Added Kestra Bitworld link to ID %s" % janeway_id),
            user=request.user,
        )
...
def production_unlink(request):
    ...
    links = ProductionLink.objects.filter(
        link_class="KestraBitworldRelease", parameter=janeway_id, production=production
    )
    if links:
        Edit.objects.create(
            action_type="production_delete_external_link",
            focus=production,
            description=("Deleted Kestra Bitworld link to ID %s" % janeway_id),
            user=request.user,
        )
        links.delete()
...
def production_import(request):
    ...
    Edit.objects.create(
        ...
        focus=production,
        description=("Added production '%s' from Janeway import" % production.title),
        user=request.user,
    )

    return render(
        request,
        "janeway/_matched_production.html",
        {
            "dz_id": production.id,

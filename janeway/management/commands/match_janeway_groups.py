    ...
    def handle(self, *args, **kwargs):
        ...
        for index, author in enumerate(Author.objects.filter(is_group=True)):
            ...
            matching_releaser_ids = set(
                DZMembership.objects.filter(group_id__in=candidate_releaser_ids, member_id__in=member_demozoo_ids)
                .distinct()
            ...
            for candidate_releaser_id in candidate_releaser_ids:
                ...
                name_match_count = (
                    DZMembership.objects.filter(
                        group_id=candidate_releaser_id, member__nicks__variants__search_title__in=member_names
                    )
                    .distinct()

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for index, author in enumerate(Author.objects.filter(is_group=False)):
            ...
            matching_releaser_ids = set(
                DZMembership.objects.filter(member_id__in=candidate_releaser_ids, group_id__in=group_demozoo_ids)
                .distinct()
            ...
            for candidate_releaser_id in candidate_releaser_ids:
                ...
                name_match_count = (
                    DZMembership.objects.filter(
                        member_id=candidate_releaser_id, group__nicks__variants__search_title__in=group_names
                    )
                    .distinct()
                ...
                if name_match_count >= 2:
                    print(
                        "scener match: %s (%d) matches %d on %d group names"

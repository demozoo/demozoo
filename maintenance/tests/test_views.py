...
class TestReports(TestCase):
    ...
    def test_unresolved_screenshots(self):
        Production.objects.get(title="Pondlife").links.create(
            link_class="BaseUrl",
            parameter="http://example.com/pondlife.zip",
    ...
    def test_fix_release_dates(self):
        ...
        self.assertEqual(Production.objects.get(title="Madrielle").release_date_date, datetime.date(2000, 1, 1))
    ...
    def test_resolve_screenshot_nonadmin(self, create_screenshot_from_production_link):
        ...
        response = self.client.post(
            "/maintenance/resolve_screenshot/%d/%d/" % (pondlife_link.id, archive_member.id), {"1": 1}
        )
        self.assertRedirects(response, "/")
    ...
    def test_fix_prod_info_file_encoding(self):
        ...
        info_file = InfoFile.objects.create(
            production=prod,
            file=File(name="pondlife.nfo", file=BytesIO(b"a g\xf6\xf6se bit me \xf6nce\n")),
            filename="pondlife.nfo",
            filesize=100,
            sha1="1234123412341234",
        )
    ...
    def test_fix_bbs_text_ad_encoding(self):
        ...
        text_ad = TextAd.objects.create(
            bbs=bbs,
            file=File(name="starport.nfo", file=BytesIO(b"a \x1b[32mj\xf6\xf6ssi\x1b[37m bit me \xf6nce\n")),
            filename="starport.nfo",
            filesize=100,
            sha1="1234123412341234",
        )
    ...
    def test_janeway_authors_same(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            Releaser.objects.get(name="Raww Arse")
            .external_links.filter(link_class="KestraBitworldAuthor", parameter="123")
            .exists()
        )
    ...
    def test_janeway_authors_different(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            Releaser.objects.get(name="Spaceballs")
            .external_links.filter(link_class="KestraBitworldAuthor", parameter="123")
            .exists()
        )
